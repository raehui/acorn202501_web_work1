...
public class PostDao {
    ...
    public int getCount(PostDto dto) {
        ...
        try {
            ...
            sql.append("SELECT COUNT(*) AS count FROM diarys");
    ...
    public boolean insert(PostDto dto) {
        ...
        try {
            conn = new DbcpBean().getConn();
            String sql = """
                INSERT INTO diarys (num, writer, title, content)
                VALUES (diarys_seq.NEXTVAL, ?, ?, ?)
            """;
            pstmt = conn.prepareStatement(sql);
    ...
    public List<PostDto> getList(PostDto dto) {
        ...
        try {
            ...
            sql.append("(SELECT num, writer, title, viewCount, TO_CHAR(createdAt, 'YYYY.MM.DD HH24:MI') AS createdAt FROM diarys ");
            //만일 검색 조건이 있다면 
            //isEmpty 는 빈문자열
            if (dto.getCondition() != null && dto.getKeyword() != null && !dto.getKeyword().isEmpty()) {
