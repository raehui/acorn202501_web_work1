	...
	<script>
		...
		function refreshComments(){
			...
			.then(commentData=>{
				...
				//댓글 목록에 있는 댓글 정보 하나 하나를 참조하면서 
				//map으로 만들었기에 list로 한 번 더 들어가야 한다.
				commentData.list.forEach(item=>{
					//댓글 하나의 정보를 makeList 함수에 전달해서 댓글 정보가 출력된 li 를 얻어낸다.
					const li = makeList(item);
					//얻어낸 li 요소를 ul 에 추가한다.
		...
		function makeList(comment){
			...
			//새로고침 해도 삭제된 댓글이면 삭제된 댓글입니다 라고 나옴
			//만일 삭제된 댓글이라면
			if(comment.deleted =="yes"){
				li.innerHTML="<p>삭제된 댓글입니다.</p>";
				//삭제된 댓글입니다가 출력된 li 를 바로 리턴해준다.
				return li;
			}
			// 프로필 이미지 처리
			// 프로필 이미지가 있다면 그 이미지로, 없다면 기본 이미지로
            ...
            //수정 삭제 링크 처리
            //로그인 회원과 작성자가 같으면 수정, 삭제 링크가 나타나겠끔 
			...
			li.innerHTML=`
				...
				<dl>
					...
					<dd>
						<!--pre 요소는 개행기호까지 출력해준다. 작성한 내용을 그대로 보여주기위해서 사용했다.-->
			...
			`;
				...
				li.querySelector(".update-form").addEventListener("submit", (e)=>{
					...
					.then(res=>res.json())
					.then(data=>{
						if(data.isSuccess){ //성공인 경우 li 안의 내용을 변경하기
			...
			<!--li 요소 안에 존재하는 것 중에 찾기 -->
			<!--답글 버튼을 누르면 위에 숨겨놓은 폼을 보여준다.-->
			...
			<!--수정 버튼을 누르면 위에 숨겨놓은 폼을 보여준다.-->
			<!--update-link는 댓글의 작성자와 로그인한 회원이 같아야 생긴다.-->
			// li.querySelector(".update-link")가 null 이 아니라면
			//자바스크립트는 데이터가 존재하면 true이고 아니면 false 로 본다.
			//update-link가 없으면 이벤트를 등록할 수 없습니다.
			
			/*
				댓글의 작성자와 로그인 회원의 정보가 다르면 수정 버튼이 생기지 않기에 생기지 않으면 이벤트를 등록할 수 없도록
			*/
