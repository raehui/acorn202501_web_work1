	...
	<script>
		...
		document.querySelector("#email").addEventListener("input",(event)=>{
			...
			if(reg_email.test(email)){
				isEmailValid=true;
				event.target.classList.add("is-valid"); //이메일 입력창에 class="is-valid"가 추가됨 
			}else{
				isEmailValid=false;
				event.target.classList.add("is-invalid"); //이메일 입력창에 class="is-invalid"가 추가됨 => invalid-feeback 요소가 화면에 나타남
			}
			checkForm();
		});
